...
import {
  CheckCircle, X, Sparkles, ArrowRight, RefreshCw, Trophy
} from "lucide-react";
...
const levels: Level[] = [
  { 
    id: 1, 
    code: "x = 5 * 4 + 2",
    question: "After constant folding?",
    options: ["x = 22", "x = 20 + 2", "x = 5 * 4 + 2"], 
    correct: 0 
  },
  { 
    id: 2, 
    code: "x = 5\nif (false):\n  x = 10\nprint(x)",
    question: "After DCE?",
    options: ["Same", "x = 5; print(x)", "print(5)"], 
    correct: 1 
  },
  { 
    id: 3, 
    code: "for i in range(3):\n  print(i)",
    question: "After loop unrolling?",
    options: ["print(0); print(1); print(2)", "Same", "i=0; print; i=1; print..."], 
    correct: 0 
  },
];

export default function OptimizationMinigameEnhanced() {
    ...
    <div className="w-full h-full flex flex-col bg-[#0d1117] rounded-lg overflow-hidden">
      <div className="p-4 border-b border-[#30363d] bg-[#161b22]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Sparkles size={18} className="text-[#3fb950]" />
            <span className="text-white font-medium">Code Optimizer</span>
          </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8">
        <div className="p-4 bg-[#21262d] rounded-lg border border-[#30363d] mb-6 w-full max-w-md">
          <pre className="text-[#c9d1d9] font-mono text-sm">{level.code}</pre>
        </div>
          ...
          {level.options.map((opt, i) => (
            ...
            >
              <div className="flex items-center gap-3">
                <pre className="text-white font-mono text-sm">{opt}</pre>
                {showResult && i === level.correct && (
