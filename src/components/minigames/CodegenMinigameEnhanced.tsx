...
const levels: Level[] = [
  { 
    id: 1, 
    ir: "t1 = a + b\nt2 = t1 * c",
    question: "If we have 2 registers (rax, rbx), how many spills?",
    options: ["0", "1", "2"], 
    correct: 0 
  },
  { 
    id: 2, 
    ir: "t1 = a + b\nt2 = c + d\nt3 = t1 + t2",
    question: "Min registers needed (no spills)?",
    options: ["2", "3", "4"], 
    correct: 1 
  },
  { 
    id: 3, 
    ir: "t1 = load x\nt2 = t1 + 1\nstore t2, x",
    question: "How many memory accesses?",
    options: ["1", "2", "3"], 
    correct: 1 
  },
];

export default function CodegenMinigameEnhanced() {
    ...
    <div className="w-full h-full flex flex-col bg-[#0d1117] rounded-lg overflow-hidden">
      <div className="p-4 border-b border-[#30363d] bg-[#161b22]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Cpu size={18} className="text-[#f0883e]" />
            <span className="text-white font-medium">Register Allocator</span>
          </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8">
        <div className="p-4 bg-[#21262d] rounded-lg border border-[#30363d] mb-6 w-full max-w-md">
          <pre className="text-[#c9d1d9] font-mono text-sm">{level.ir}</pre>
        </div>
