...
import {
  CheckCircle, X, AlertTriangle, ArrowRight, RefreshCw, Trophy
} from "lucide-react";
...
const levels: Level[] = [
  { 
    id: 1, 
    stack: ["main()", "process() [catch]", "parse()", "validate()"],
    throwAt: "validate()",
    question: "Where is the exception caught?",
    options: ["main()", "process()", "parse()", "validate()"], 
    correct: 1 
  },
  { 
    id: 2, 
    stack: ["main()", "run()", "execute()", "check()"],
    throwAt: "check()",
    question: "What happens?",
    options: ["Caught in run()", "Caught in main()", "Program terminates"], 
    correct: 2 
  },
  { 
    id: 3, 
    stack: ["main() [catch]", "funcA()", "funcB() [catch]", "funcC()"],
    throwAt: "funcC()",
    question: "Where is the exception caught?",
    options: ["main()", "funcA()", "funcB()"], 
    correct: 2 
  },
];

export default function ExceptionMinigameEnhanced() {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-[#0d1117] rounded-lg overflow-hidden">
      <div className="p-4 border-b border-[#30363d] bg-[#161b22]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-medium">Stack Unwinding</span>
          </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8">
        <div className="text-[#8b949e] text-sm mb-4">Call Stack (top to bottom)</div>
        <div className="space-y-2 mb-6 w-full max-w-md">
          {level.stack.map((func, i) => (
            <div 
              key={i} 
              className={`p-3 rounded-lg border text-center ${
                func.includes(level.throwAt) 
            ...
            >
              <span className={func.includes(level.throwAt) ? "text-[#f85149] font-bold" : "text-[#c9d1d9]"}>
                {func}
              </span>
            </div>
          ))}
